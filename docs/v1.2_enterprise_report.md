...
**New:** `src/ai/index_backends.py`, `src/ai/index_advisor.py`, `src/core/compatibility.py`, `tests/test_v12_enterprise.py`, `tests/test_v12_stress.py`, `tests/test_index_advisor.py`, `dev_tools/benchmark_index_backends.py`, this report  
...
### Index Advisor (Settings → Index Backend)

Read-only recommendation engine (`advise_index_backend`). Never auto-switches.

| Catalog | Recommendation |
|---------|----------------|
| &lt;100k | FlatIP |
| 100k–500k | FlatIP (+ optional HNSW) |
| 500k–1M | HNSW |
| &gt;1M | HNSW; IVF-PQ only if RAM insufficient |

UI shows Recommended Backend, Reason, Estimated RAM, Expected Recall, Expected Search Time, Rebuild Required, comparison table, and Apply Recommendation (confirm required).

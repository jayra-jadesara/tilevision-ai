...
class AppSettings:
    ...
    def _enforce_production_index_backend(self) -> None:
        """Force production backend to flat_ip (exact search)."""
    ...
    def enable_sam2_precise_crop(self, value: bool) -> None:
        self._settings["enable_sam2_precise_crop"] = bool(value)
        self.save()
    ...
    def index_backend(self) -> str:
        """Production backend is always flat_ip (exact)."""
        return "flat_ip"

    @index_backend.setter
    def index_backend(self, value: str) -> None:
        """
        Production ignores non-flat backends.

        Developer Mode may write a temporary override into the raw settings
        dict for rebuild experiments; the public getter still reports flat_ip
        unless developer tooling reads `_settings` directly.
        """
        from src.ai.index_backends import IndexBackend

        parsed = IndexBackend.parse(value).value
        if parsed != "flat_ip" and os.environ.get("TILEVISION_DEV_MODE") != "1":
            parsed = "flat_ip"
        self._settings["index_backend"] = parsed
        self.save()
